...
library(lattice)
library(gdata)
opts_chunk$set(echo=FALSE, fig.width=9, message = FALSE, warnings = FALSE)
```
# Introduction
...
* Large dataset (approx. 3 million rows) but would fit in my laptop’s memory
...
# Load the Data
load("/Users/alexspanos/Documents//Nanodegree/EDA/project/project.RData")
```
...
Please see relevant scripts * [here](https://github.com/alejio/Udacity-EDA-draft).

```{r eval=FALSE, code = readLines('/Users/alexspanos/Documents//Nanodegree/EDA/project/R/data_load_main.R')}
```

```{r eval=FALSE, code = readLines('/Users/alexspanos/Documents//Nanodegree/EDA/project/R/data_load_aux.R')}
```

```{r eval=FALSE, code = readLines('/Users/alexspanos/Documents//Nanodegree/EDA/project/R/data_merge.R')}
```
## Dataset features
...
```{r echo=FALSE, Dataset_Features1}
...
```{r echo=FALSE, Dataset_Features2}
...
```{r echo=FALSE, quickhistliq1, warning=FALSE, cache=TRUE, cache.path='cache/', fig.path='figure/'}
...
```{r echo=FALSE, quickhistliq2, warning=FALSE, cache=TRUE, cache.path='cache/', fig.path='figure/'}
...
multiplot(quick_hist(14, 2) + coord_cartesian(xlim=c(0, 50)), 
          ...
          quick_hist(16,0.1) + scale_x_log10(), 
          quick_hist(17, 0.1) + scale_x_log10(), 
...
```{r echo=FALSE, quickhistmeteo1, warning=FALSE, cache=TRUE, cache.path='cache/', fig.path='figure/'}
...
```{r echo=FALSE, quickhistmeteo2, warning=FALSE, cache=TRUE, cache.path='cache/', fig.path='figure/'}
...
```{r echo=FALSE, quickhistdemo1, warning=FALSE, cache=TRUE, cache.path='cache/', fig.path='figure/'}
...
gr_cat <- ddply(data1, .(`CATEGORY NAME`), summarise,
                ...
                price=mean(`BTL PRICE`))
```

```{r echo=FALSE, gr_cat_show}
...
```{r echo=FALSE, gr_item_show_most}
#Most popular
...
```{r echo=FALSE, gr_item_show_least}
#Least popular
arrange(gr_item, bottles)[1:30,]
```
...
```{r echo=FALSE, ggpairs1, fig.width=12, fig.height=9, warning=FALSE, cache=TRUE, cache.path='cache/', fig.path='figure/'}
ggpairs(gr_zip_pop, columns=c(8,9,10,14,4,5,22),
                 upper = "blank", lower=list(params=list(size=1)),
                 params = c(Shape = I("."), outlier.shape = I(".")), 
                 title="Scatter plots between national heritage 
        group proportions") + 
...
```{r echo=FALSE, ggpairs2, fig.width=12, fig.height=9, warning=FALSE, cache=TRUE, cache.path='cache/', fig.path='figure/'}
ggpairs(pairs2, 
        upper = "blank", lower=list(params=list(size=1)),
        params = c(Shape = I("."), outlier.shape = I("."))) + 
  theme(legend.position = "none",
...
```{r echo=FALSE, Demo_plots1, warning=FALSE, cache=TRUE, cache.path='cache/', fig.path='figure/'}
...
```{r echo=FALSE, Demo_plots2, warning=FALSE, cache=TRUE, cache.path='cache/', fig.path='figure/'}
IowaMap + 
...
```{r echo=FALSE, Demo_plots3, cache=TRUE, cache.path='cache/', fig.path='figure/'}
IowaMap + 
...
```{r echo=FALSE, Demo_plots4, cache=TRUE, cache.path='cache/', fig.path='figure/'}

#Produce maps with overlays for all nationalities

suppressWarnings(multiplot(plot_map_nat_over(3), plot_map_nat_over(6), 
                           plot_map_nat_over(7), 
...
rm(test)
```
```{r echo=FALSE, Demo_plots5, warning=FALSE, cache=TRUE, cache.path='cache/', fig.path='figure/'}
#Plot
suppressWarnings(IowaMap + 
...
```{r echo=FALSE, auditplot1, cache=TRUE, cache.path='cache/', fig.path='figure/'}
...
ggplot(data=gr_st_month_count, aes(x=month, y=count_reps)) + 
  ...
  geom_boxplot(aes(fill=part_stores), outlier.size=0, alpha=0.6) +
  scale_fill_gradientn("# of \n participating \n stores", colours=topo.colors(10))+
...
gr_st_week.part_stores <- data.frame("week"=
                                       rownames(table(gr_st_week_count$week)),
  "part_stores"=as.numeric(table(gr_st_week_count$week)))
gr_st_week.part_stores$week <- 
  as.numeric(as.character(gr_st_week.part_stores$week))
...
```{r echo=FALSE, auditplot2, cache=TRUE, cache.path='cache/', fig.path='figure/'}
...
gr_st_day.part_stores <- data.frame("day"=rownames(table(gr_st_day_count$day)),
                                     "part_stores"=
                                      as.numeric(table(gr_st_day_count$day)))
...
```{r echo=FALSE, auditplot3, cache=TRUE, cache.path='cache/', fig.path='figure/'}
...
gr_st_weekday_count$weekday <- factor(gr_st_weekday_count$weekday, 
                                      levels=day.name)
gr_st_weekday.part_stores <- 
  data.frame("weekday"=rownames(table(gr_st_weekday_count$weekday)),
                                    "part_stores"=as.numeric(
                                      table(gr_st_weekday_count$weekday)))
gr_st_weekday.part_stores$weekday <- factor(gr_st_weekday.part_stores$weekday,
                                            levels=day.name)
...
```{r echo=FALSE, auditplot4, cache=TRUE, cache.path='cache/', fig.path='figure/'}
#Plot data
...
```{r echo=FALSE, Liqplots1, cache=TRUE, cache.path='cache/', fig.path='figure/'}
...
```{r echo=FALSE, Liqplots2, cache=TRUE, cache.path='cache/', fig.path='figure/'}
ggplot(data=data1, aes(x=`BOTTLE QTY`)) +
  geom_histogram(aes(y =..density..),
                 fill=I("blue"), col=I("black"), alpha=0.5, origin = -0.5) +
  labs(title="Histogram for Log Bottle Quantities", x="Log Bottles", 
       y="Count") +
  scale_x_log10(breaks=c(1, 5, 10, 25, 50, 100, 150, 250, 400)) +
  theme(panel.background = element_rect(fill = 'white'),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), 
        plot.title = element_text(size = rel(1), colour = "blue"))
...
```{r echo=FALSE, Liqplots3, cache=TRUE, cache.path='cache/', fig.path='figure/'}
...
```{r echo=FALSE, Liqplots4, fig.width=12, fig.height=9, cache=TRUE, cache.path='cache/', fig.path='figure/'}
ggplot(data=gr_date.Jan_May,aes(x=DATE, y=bottles/maxbottles, colour=type)) + 
  geom_point()+ geom_line() +
                            geom_ribbon(aes(ymin=0, ymax=bottles/maxbottles, 
                                            fill=type)) +
...
```{r echo=FALSE, Liqplots5, cache=TRUE, cache.path='cache/', fig.path='figure/'}
...
```{r echo=FALSE, Liqplots6, cache=TRUE, cache.path='cache/', fig.path='figure/'}
ggplot(data=gr_date.Jan_May,aes(x=bottles[type=="BOTTLES"]/
                                  maxbottles[type=="BOTTLES"], 
                                y=bottles[type=="BOTTLES_NORM"]/
                                  maxbottles[type=="BOTTLES_NORM"],
                                shape=weekdays(DATE[type=="BOTTLES"]),
                                colour=weekdays(DATE[type=="BOTTLES"]))) +
  geom_point(size=3) + 
  labs(x="raw quantities", y="population adjusted quantities")
```
...
```{r echo=FALSE, Liqplots7, cache=TRUE, cache.path='cache/', fig.path='figure/'}
#plot raw
...
```{r echo=FALSE, Liqplots8, cache=TRUE, cache.path='cache/', fig.path='figure/'}
...
```{r echo=FALSE, Liqplots8b, cache=TRUE, warning=FALSE,cache.path='cache/', fig.path='figure/'}
...
#Plot
ggplot(data=gr_item, aes(x=price, y=bottles)) + geom_point(alpha=0.3) +
  scale_y_log10() + scale_x_log10(limits=c(5,250))+ geom_smooth(method="lm",
                                                                fill="red") +
...
gr_date_cat2 <- ddply(data1, .(DATE, `CATEGORY NAME`), summarise, 
                      BOTTLES = sum(`BOTTLE QTY`, na.rm=T))
gr_date_cat2.month <- ddply(gr_date_cat2, .(factor(months(DATE), 
                                                   levels=month.name),
...
gr_date_cat2.month$BCAT_NORM <-
  gr_date_cat2.month$BOTTLES/gr_date_cat2.month$TOT_BOTT
...
gr_date_cat2.month <- mutate(
  gr_date_cat2.month, 
  month_nom = 
    ifelse(Month=="January", dec_dates[1], 
...
```{r echo=FALSE, Liqplots9, fig.width=12, fig.height=9, cache=TRUE, cache.path='cache/', fig.path='figure/'}
# Settings for ggplot2
txmin <- decimal_date(as.Date(c("2014-01-01", "2014-03-01", "2014-06-01",
                                "2014-09-01", "2014-12-01")))
txmax <- c(decimal_date(as.Date(c("2014-03-01", "2014-06-01", "2014-09-01",
                                  "2014-12-01"))), Inf)
tymin <- rep(-Inf, 5)
tymax <- rep(Inf, 5)
...
colScale <- c(col2hcl("blue"), col2hcl("green"), col2hcl("red"), 
              col2hcl("purple"), col2hcl("blue"))
...
gr_date_cat2.month_v3 <- mutate(gr_date_cat2.month_v3, 
       season=ifelse(Month %in% c("January", "February", "December"), 
                     "2.winter", 
                     ifelse(Month %in% c("March", "April", "May"), "3.spring",
                            ifelse(Month %in% c("June", "July", "August"), 
                                   "1.summer", "4.autumn"))))

gr_date_cat2.month_v3 <- ddply(gr_date_cat2.month_v3, .(CATEGORY,season), 
                               summarise, BOTTLES=sum(BOTTLES, na.rm=T))

gr_date_cat2.month_v3.temp <- ddply(gr_date_cat2.month_v3, .(CATEGORY), 
                                    summarise, 
...
gr_date_cat2.month_v3 <- 
  mutate(gr_date_cat2.month_v3, 
...
gr_date_cat2.month_v4 <- ddply(gr_date_cat2.month_v3, .(broad_cat, season), 
                               summarise, 
                               BOTTLES=sum(BOTTLES, na.rm=T))

gr_date_cat2.month_v4.temp <- ddply(gr_date_cat2.month_v4, .(broad_cat), 
                                    summarise, 
                                    BOTTLES_SUM=sum(BOTTLES, na.rm=T))
...
gr_date_cat2.month_v4.temp <- ddply(gr_date_cat2.month_v4, .(season),
                                    summarise, 
                                    BOTTLES_SUM=sum(BOTTLES, na.rm=T))
...
gr_date_cat2.month_v4.temp$BOTTLES_PROP_ADJ <- 
  gr_date_cat2.month_v4.temp$BOTTLES_SUM/
  sum(gr_date_cat2.month_v4.temp$BOTTLES_SUM)
...
gr_date_cat2.month_v4 <- 
  mutate(gr_date_cat2.month_v4, 
     Seasonal_Profile=ifelse(Type %in% c("Vodkas", "Tequila", "Gins", "Rums"), 
                             "Warm", "Cold"))
```


```{r echo=FALSE, Liqplots10, fig.width=12, fig.height=9, cache=TRUE, cache.path='cache/', fig.path='figure/'}
...
ggplot(data=gr_date_cat2.month_v4, aes(x=season, y=BOTTLES_ADJ,  group=Type, colour=Seasonal_Profile)) +
  geom_line(linetype=2, size=1) +  geom_point(size=3) + 
  scale_x_discrete(limits=c("2.winter", "3.spring", "1.summer", "4.autumn"))+
  facet_wrap(~Type, scales="free_y") + 
  labs(x="Season", y="Seasonally adjusted bottle sales ratio",
       title = "Seasonal profile of main liquor categories")+
...
```{r eval=FALSE, echo=FALSE, corr_cat_ts}
corr.cat.ts <- rcorr(as.matrix(gr_date_cat.spread[,c(3:70)]))$r
corr.cat.ts <- data.frame(corr.cat.ts)
```

```{r echo=FALSE, corr_heat_map, fig.width=12, fig.height=9, warning=FALSE, cache=TRUE, cache.path='cache/', fig.path='figure/'}
z <- as.matrix(corr.cat.ts)
#lowerTriangle(z, diag=T) <- ""
levelplot(z,scales=list(x=list(rot=60, abbreviate=TRUE, minlength=8, tck=0.3,
                               alternating=1, cex=0.7),
                        y=list(tck=0.3)),
          main="Correlation matrix of categories",col.regions= topo.colors, 
          cuts=4, at=seq(-1,1,0.4), xlab="", ylab="",
          colorkey=list(labels=list(at=seq(-0.8,0.8,0.4), 
                                    labels=c("High Inverse", "Med Inverse", 
                                             "Insignificant", "Med Positive",
                                             "High Positive"))))
```
...
```{r echo=FALSE, corr_heat_map_v2, fig.width=12, fig.height=9, warning=FALSE, cache=TRUE, cache.path='cache/', fig.path='figure/'}
ind.corr1 <- NULL
ind.corr2 <- NULL
z <- corr.cat.ts
z[] <- NA
for (i in 1:69){
  ind.corr1 <- as.vector(apply(corr.cat.ts[i,], 1, 
                               function(x) order(x, decreasing=T)[2]))
  #print(ind.corr)
  z[i,ind.corr1] <- c(1)
  ind.corr2 <- as.vector(apply(corr.cat.ts[i,], 1, 
                               function(x) order(x, decreasing=F)[2]))
  #print(ind.corr)
  z[i,ind.corr2] <- c(-1)
}
z <- z[-nrow(z),]
z <- as.matrix(z)
heatmap(z,Rowv=NA, Colv=NA, col = cm.colors(2), scale="none", na.rm=T, labCol=abbreviate(rownames(z), minlength=5), 
        main="Top Positive and Negative correlations",
        add.expr = grid(68,68))
```
...
```{r echo=FALSE, TEQcorrs, fig.width=12, fig.height=9, warning=FALSE, cache=TRUE, cache.path='cache/', fig.path='figure/'}
#Produce plots

test1 <- ggplot(data=gr_date_cat.spread[,c(64,44)],
                aes(x=`MISC. IMPORTED CORDIALS & LIQUEURS`, y=TEQUILA)) +
  geom_point() +
...
test2 <- ggplot(data=gr_date_cat.spread[,c(64,65)],
                aes(x=`TRIPLE SEC`, y=TEQUILA)) +
  geom_point() +
  geom_smooth(method=lm) +  
  geom_text(x = 1500, y = 10000, 
                label = lm_eqn(lm(TEQUILA~`TRIPLE SEC`,
                                  gr_date_cat.spread[c(64,65)])),
                parse = TRUE, size = 3)

test3 <- ggplot(data=gr_date_cat.spread[,c(64,39)],
                aes(x=`IMPORTED VODKA - MISC`, y=TEQUILA)) +
  geom_point() +
  geom_smooth(method=lm) +  
  geom_text(x = 1500, y = 10000, 
            label = lm_eqn(lm(TEQUILA~`IMPORTED VODKA - MISC`,
                              gr_date_cat.spread[c(64,39)])),
                                 parse = TRUE, size = 3)


test4 <- ggplot(data=gr_date_cat.spread[,c(64,33)],
                aes(x=`HIGH PROOF BEER`, y=TEQUILA)) +
  geom_point() +
  geom_smooth(method=lm, colour="red") +  
  geom_text(x = 5, y = 15000, 
            label = lm_eqn(lm(TEQUILA~`HIGH PROOF BEER`,
                              gr_date_cat.spread[c(64,33)])),
                                parse = TRUE, size = 3)

test5 <- ggplot(data=gr_date_cat.spread[,c(64,48)],
                aes(x=`PEACH BRANDIES`, y=TEQUILA)) +
  geom_point() +
  geom_smooth(method=lm, colour="red") +  
  geom_text(x = 500, y = 10000, 
            label = lm_eqn(lm(TEQUILA~`PEACH BRANDIES`,
                              gr_date_cat.spread[c(64,48)])),
                               parse = TRUE, size = 3)

test6 <- ggplot(data=gr_date_cat.spread[,c(64,26)],
                aes(x=`DECANTERS & SPECIALTY PACKAGES`, y=TEQUILA)) +
  geom_point() +
  geom_smooth(method=lm, colour="red") +  
...
```{r echo=FALSE, VODcors, fig.width=12, fig.height=9, warning=FALSE, cache=TRUE, cache.path='cache/', fig.path='figure/'}

test1 <- ggplot(data=gr_date_cat.spread[,c(4,59)],
                aes(x=`SPICED RUM`, y=`80 PROOF VODKA`)) +
  geom_point() +
  geom_smooth(method=lm) +  
  geom_text(x = 1500, y = 40000, 
            label = lm_eqn(lm(`80 PROOF VODKA`~`SPICED RUM`,
                              gr_date_cat.spread[c(4,59)])),
            parse = TRUE, size = 3)

test2 <- ggplot(data=gr_date_cat.spread[,c(4,51)],
                aes(x=`PUERTO RICO & VIRGIN ISLANDS RUM`, y=`80 PROOF VODKA`)) +
  geom_point() +
  geom_smooth(method=lm) +  
  geom_text(x = 1500, y = 40000, 
...
test3 <- ggplot(data=gr_date_cat.spread[,c(4,60)],
                aes(x=`STRAIGHT BOURBON WHISKIES`, y=`80 PROOF VODKA`)) +
  geom_point() +
  geom_smooth(method=lm) +  
  geom_text(x = 1500, y = 40000, 
...
test4 <- ggplot(data=gr_date_cat.spread[,c(4,33)],
                aes(x=`HIGH PROOF BEER`, y=`80 PROOF VODKA`)) +
  geom_point() +
  geom_smooth(method=lm, colour="red") +  
  geom_text(x = 5, y = 15000, 
            label = lm_eqn(lm(`80 PROOF VODKA`~`HIGH PROOF BEER`,
                              gr_date_cat.spread[c(4,33)])),
            parse = TRUE, size = 3)

test5 <- ggplot(data=gr_date_cat.spread[,c(4,48)],
                aes(x=`PEACH BRANDIES`, y=`80 PROOF VODKA`)) +
  geom_point() +
  geom_smooth(method=lm, colour="red") +  
  geom_text(x = 500, y = 10000, 
            label = lm_eqn(lm(`80 PROOF VODKA`~`PEACH BRANDIES`,
                              gr_date_cat.spread[c(4,48)])),
            parse = TRUE, size = 3)

test6 <- ggplot(data=gr_date_cat.spread[,c(4,26)],
                aes(x=`DECANTERS & SPECIALTY PACKAGES`, y=`80 PROOF VODKA`)) +
  geom_point() +
  geom_smooth(method=lm, colour="red") +  
  geom_text(x = 5000, y = 12000, 
...
gr_date.Jan_May_v2 <- gr_date.Jan_May
gr_date.Jan_May_v2$type <-
  sapply(gr_date.Jan_May_v2$type, 
         function(x) ifelse(x=="BOTTLES", "1. raw quantities", 
                            "2. population-adjusted quantities"))
ggplot(data=gr_date.Jan_May_v2,
       aes(x=DATE, y=bottles/maxbottles, colour=type)) + 
  geom_point()+ geom_line() +
  geom_ribbon(aes(ymin=0, ymax=bottles/maxbottles, fill=type)) +
  scale_fill_brewer(palette="Set3")+
  labs(x="Date", y = "Bottles", 
       title = "Time-series of bottle quantities ordered")+
  theme(panel.background = element_rect(fill = 'white'),
        plot.title = element_text(size = rel(1), colour = "blue")) +
  geom_vline(xintercept=
               as.numeric(gr_date.Jan_May[weekdays(gr_date.Jan_May$DATE)==
                                            "Tuesday",]$DATE), linetype=4, 
                                              alpha=0.4)
```
