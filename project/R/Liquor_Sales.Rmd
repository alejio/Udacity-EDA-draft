...
z[] <- NA
for (i in 1:69){
  ind.corr1 <- as.vector(apply(corr.cat.ts[i,], 1, function(x) order(x, decreasing=T)[2]))
  #print(ind.corr)
  z[i,ind.corr1] <- c(1)
  ind.corr2 <- as.vector(apply(corr.cat.ts[i,], 1, function(x) order(x, decreasing=F)[2]))
  #print(ind.corr)
  z[i,ind.corr2] <- c(-1)
}
z <- z[-nrow(z),]
...
gr_date.Jan_May_v2$type <- sapply(gr_date.Jan_May_v2$type, function(x) ifelse(x=="BOTTLES", "1. raw quantities", "2. population-adjusted quantities"))
ggplot(data=gr_date.Jan_May_v2,aes(x=DATE, y=bottles/maxbottles, colour=type)) + 
  geom_point()+ geom_line() +
  geom_ribbon(aes(ymin=0, ymax=bottles/maxbottles, fill=type)) +
  scale_fill_brewer(palette="Set3")+
